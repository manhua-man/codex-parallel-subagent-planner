...
Assign each lane a clear descriptive role (e.g., `explorer`, `implementer`, `reviewer`, `migrator`, or any domain role suited to the task), `Read`, `Write`, `Ignore` (system noise + sibling scopes), and model profile:
